...
A lightweight bridge between domain exceptions and validation violations.

![Exceptional Matcher.svg](https://raw.githubusercontent.com/phphd/exceptional-validation/refs/heads/main/assets/Exceptional%Matcher.svg)

Throw domain exceptions from your code (services / value objects),
and use this library to match them to property paths and format as ConstraintViolations (or your chosen format) –
so that your backend can return field-specific (precise) errors inferred from these exceptions.
...
Here, `LoginAlreadyTakenException` is related to the `login` property, \
while `PasswordCompromisedException` is related to the `password` property.

> You can have extra conditions for exception matching beyond just class name. \
> See [Match Conditions 🖇️](docs/config/match-conditions.md).

The equivalent (very simplified) manual logic if not using this library:
...
class RegisterUserApiPoint
{
    ...
    public function __invoke(RegisterUserDto $dto): Response
    {
        try {
            ...
            return $this->validationError($exception, $dto);
        }
    }

    private function validationError(\Exception $exception, RegisterUserDto $dto): Response
    {
...
This's what we'd have to do with an upfront attribute-driven validation:
...
Compare it to this `#[Catch_]` and discern which communicates the intent better.
...
exceptions**. \
It makes up for what was lacking in tools for relating these exceptions to their originator fields.

With Exceptional Matcher you can **omit** any **peripheral validation** off your dto objects, \
and rely solely on validation in real code (services, value objects) – that resides in and belongs to the domain.
...
Finding a match for the exception in `array` field is like finding your luggage at the airport \
when everyone else took just the same very alike red backpack as you did.
...
Matching of `BackorderDisabledForCategoryException` might have related it to any object of the original array, \
given they all define the same class name condition as the desired item (e.g. the same color).
...
- [Matching multiple exceptions 🕎](docs/multi-match/matching-multiple-exceptions.md) – go beyond just one
  thrown exception.
