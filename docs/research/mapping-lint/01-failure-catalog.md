...
| B10 | `format:` formatter class not registered | **post-match** (`DelegatingMatchedExceptionFormatter`) — even later than assembly | `DelegatingMatchedExceptionFormatter::format()` (`has()` + `LogicException`) | plan compiler checks `has($catch->getFormat())` against the same tagged-locator registry, reported as a **error**; the formatter-side check remains the runtime enforcement |
...
|---|---|---|---|---|
| C1 | Properties carry `#[Catch_]` but the class lacks `#[Try_]` | **silent** — the whole class is skipped (`ObjectMatchingRuleSetAssembler::isMarkedWithAnAttribute()`) | statically | plan compiler structural pass — **error**, reported only when the compiler is invoked directly (lint); at runtime `PlanRegistry` returns `null` for classes without `Try_` *before* compiling, preserving today's silent-skip behavior |
...
| C3 | `#[Try_]` with zero `#[Catch_]` properties | **silent** as a direct match target — but such a class may still match through *nested/iterable* properties | statically | plan compiler structural pass — **warning** that never blocks plan creation (nested/iterable traversal must keep working) |
...
mapping can never succeed. These are lint **warnings** (a `mixed`/undeclared type produces no defect report).
...
| D. Type compatibility | 4 | 1 already-enforced error (D4), 3 warnings |
