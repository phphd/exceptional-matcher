...
For example, `CardBlockedException` could be related to `$depositToCardId` or `$withdrawFromCardId`:
...
Once `CardBlockedException`  is matched by class, custom closure is called:
...
In this example `CardBlockedException` could be linked either to `withdrawalCardId` or to `depositCardId`, \
depending on the value of `cardId` from the exception.

And `CardBlockedException` must implement `ValueException` interface:

```php
use PhPhD\ExceptionalMatcher\Rule\Object\Property\Match\Condition\Value\ValueException;

class CardBlockedException extends RuntimeException implements ValueException
{
